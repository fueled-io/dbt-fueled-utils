...
{% for i in range(test_data.min_last_success|length) %}
  ...
  {%- set results = run_query(fueled_utils.get_run_limits(test_data.min_last_success[i],
  ...
  select
    {{fueled_utils.cast_to_tstamp(expected_lower_limit)}} as expected_lower_limit,
    {{fueled_utils.cast_to_tstamp(expected_upper_limit)}} as expected_upper_limit,
    {{fueled_utils.cast_to_tstamp(actual_lower_limit)}} as actual_lower_limit,
    {{fueled_utils.cast_to_tstamp(actual_upper_limit)}} as actual_upper_limit
